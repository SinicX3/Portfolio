import Carousel from "../../Shared Components/Carousel"

function Site() {

  const KasaImg = ['/sites/Kasa (1).png', '/sites/Kasa (2).png', '/sites/Kasa (3).png']

  return(
    <section id="Site_section" className="panel">
      <div className="site_description">
        <img src="/sites/Kasa_logo.png" id="kasa_logo" alt="Logo de Kasa" />
        <p>Produit dans le cadre de la formation "Développeur web" d'OpenClassrooms, le projet Kasa illustre mon utilisation de React et React-Router.</p>
      </div>
      <Carousel images={KasaImg} />
      <div className="site_description">
        <div>
          <h2>Objectifs</h2>
          <ul>
            <li>Produire un site intuitif</li>
            <li>Assurer une navigation fluide et responsive</li>
            <li>Utiliser React pour la structuration des composants</li>
            <li>Respecter une maquette fournie en responsive design</li>
          </ul>
        </div>
        <div>
          <h2>Technologies utilisées</h2>
          <ul>
            <li>React, React Router</li>
            <li>Gestion d'état avec useState</li>
            <li>Gestion de contexte avec useContext</li>
            <li>GitHub</li>
          </ul>
        </div>
      </div>
    </section>
  )

  // return (
  //   <section id="Kasa_section" className="panel">
  //     <div className="site_description">
  //       <img src="/sites/Kasa_logo.png" id="kasa_logo" alt="Logo de Kasa" />
  //       <h2>Réalisation d'un site dédié à la recherche de biens immobiliers.</h2>
  //       <p>Date : Février 2025</p>
  //       <p>Produit dans le cadre de la formation OpenClassrooms, ce site illustre mes compétences avec React et React-Router, notamment sur l'utilisation des hooks et des composants</p>
  //     </div>
  //     <div className="site_img">
  //       <img src="/sites/Kasa (1).png"/>
  //     </div>
  //     <div className="objectifs">
  //       <h3>Objectifs :</h3>
  //       <ul>
  //         <li>Produire un site intuitif</li>
  //         <li>Assurer une navigation fluide et responsive</li>
  //         <li>Utiliser React pour la structuration des composants</li>
  //         <li>Respecter une maquette fournie en responsive design</li>
  //       </ul>
  //     </div>
  //     <div className="site_img">
  //       <img src="/sites/Kasa (2).png"/>
  //     </div>
  //     <div className="site_technos">
  //       <h3>Technologies utilisées :</h3>
  //       <ul>
  //         <li>React, React Router</li>
  //         <li>Gestion d'état avec useState</li>
  //         <li>Gestion de contexte avec useContext</li>
  //         <li>GitHub</li>
  //       </ul>
  //     </div>
  //     <div className="site_img">
  //       <img src="/sites/Kasa (3).png"/>
  //     </div>
  //     <div className="site_link">
  //       <a href="https://github.com/SinicX3/Projet-5---React.git" target="_blank" rel="noopener noreferrer">
  //         Retrouvez ce projet sur GitHub
  //       </a>
  //     </div>
  //   </section>
  // )
}

export default Site
